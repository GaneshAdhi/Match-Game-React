...
const Home = () => {
  ...
  const [gameCardShow, setGameshow] = useState(true)
  const [activeTabId, setTabId] = useState(tabsList[0].tabId)
  const [imageObject, setImageObject] = useState(imagesList[0])
  ...
  const fliterNewImageList = imagesList.filter(
    eachImageItem => activeTabId === eachImageItem.category,
  )

  const imageChange = id => {
    if (id === imageObject.id) {
      const filteredImages = imagesList.filter(img => img.id !== id)
      const randomImage =
        filteredImages[Math.floor(Math.random() * filteredImages.length)]
      setImageObject(randomImage)
  ...
  const resetGame = () => {
    ...
    setTime(60)
    setScore(0)
    setTabId(tabsList[0].tabId)
    setImageObject(imagesList[0])
    setGameshow(true)
  }

  return (
    <div className="home-main-container">
      <Header scoreText={score} timeText={time} />
      {gameCardShow ? (
        <div className="home-content-container">
          ...
          />
          <ul className="home-content-tab-button-container">
            {tabsList.map(eachTabItem => (
              <CategoryButtons
                changeActiveId={onActiveTabId}
                activeTabButtonStatus={activeTabId === eachTabItem.tabId}
                key={eachTabItem.tabId}
                tabItemDetail={eachTabItem}
              />
            ))}
          </ul>
          <ul className="image-button-container">
            {fliterNewImageList.map(eachImage => (
              <ImageButtons
                key={eachImage.id}
                imageDetail={eachImage}
                imageChangeOn={imageChange}
              />
            ))}
          </ul>
        </div>
      ) : (
        <ScoreCard scoreText={score} reset={resetGame} />
      )}
    </div>
  )
}
